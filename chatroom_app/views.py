...
def chat(request):
    ...
    def test_name_room_valid(name, room, password):
        name_length = len(name)
        room_length = len(room)
        password_length = len(room)

        if name_length == 0 or room_length == 0:
            ...
            messages.success(request,"Error: password too long")
            return HttpResponseRedirect(reverse('index'))
        
        else:
            return None
    
    redirect_response = test_name_room_valid(name, room)
    ...
    #Get server object
    server_object = ServerList.objects.filter(server=room)
    server_object = None if len(server_object) == 0 else server_object[0]

    #If server doesn't exist, create it
    if not server_object:
        server_object = ServerList(server=room,
                            lockStatus=False,
        ...
        #Add user to server
        user_object = UserList(username=name,
                        server=server_object,
                        time=datetime.datetime.now().astimezone())
        user_object.save()

        #Show that user joined
        chat_object = Chats(server=server_object,
                     username='admin',
        ...
        if server_object.lockStatus == True:

            if server_object.password == 'none':
                messages.success(request,"Error: Room is locked")
                return HttpResponseRedirect(reverse('index'))
            
            elif server_object.password != password:
                messages.success(request,"Error: Password needed")
                return HttpResponseRedirect(reverse('index'))

        #if username is already in use in the given server, don't allow the request
        user_list = UserList.objects.filter(server=server_object,
                                            username=name)
...
def sendmsg(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        ...
        if message_length > 0 and message_length < 100000:
            ...
            time = datetime.datetime.now().astimezone()
            server_object = ServerList.objects.get(server=room)
            chat_object = Chats(server=server_object,
                      username=name,
                      message=message,
                      time=time)
...
def sendcmd(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        ...
        try:
            command = post_data["command"].strip().lower()
            room = post_data["room"]
        except:
            return JsonResponse({})
        ...
        if command=="leave":
            ...
            if server_object.lockStatus == True:
                ServerList.objects.filter(server=room).update(lockStatus=False)
                chat_object = Chats(server=server_object,
                          username='admin',
                          message="Server unlocked.",
                          time=datetime.datetime.now().astimezone())
                chat_object.save()
            return JsonResponse({})
        ...
        #If close, remove all users, messages, server and redirect user. 
            ...
            user_list = UserList.objects.filter(server=room).values()
            user_list_length = len(user_list)

            if user_list_length == 1:

                for i in Chats.objects.filter(server=server_object).values():
                    chatID = i.get("chatID")
                    chat_object = Chats.objects.get(chatID=chatID)
                    chat_object.delete()
                ...
                chat_object = Chats(server=server_object,
                                    username='admin',
                                    message="Chatroom could not be closed.",
                                    time=datetime.datetime.now().astimezone())
                chat_object.save()
            return JsonResponse({})
        ...
        if command[0]=='lock':
            ...
            #no password
            if len(command)==1:
                if server_object.lockStatus==False:
                    ServerList.objects.filter(server=room).update(lockStatus=True,password="none")
                    chat_object = Chats(server=server_object,
                                        username='admin',
                ...
                if len(password)>16:
                    chat_object = Chats(server=server_object,
                              username='admin',
                              message=f"Server password too long.",
                              time=datetime.datetime.now().astimezone())
                    chat_object.save()
                    ...
                    if hashed_password != server_object.password:
                        ServerList.objects.filter(server=room).update(lockStatus=True,
                                                                        password=hashed_password)
                        chat_object = Chats(server=server_object,
                                  username='admin',
                                  message=f"Server password set: {password}",
                                  time=datetime.datetime.now().astimezone())
                        chat_object.save()
        ...
        #If colour, validate colour then send message to server, which will update page
            ...
            if len(command)==2:
                color = command[1]
                valid, color = validate_color(color)
                if valid:
                    color = color.upper()
                    chat_object = Chats(server=server_object,
                              username='admin',
                              message=f"Text color changed to {color}.",
                              time=datetime.datetime.now().astimezone())
                    chat_object.save()
        ...
        #If background, validate colour then send message to server, which will update page
            ...
            if len(command)==2:
                ...
                if valid:
                    ...
                    chat_object = Chats(server=server_object,
                              username='admin',
                              message=f"Background color changed to {color}.",
                              time=datetime.datetime.now().astimezone())
                    chat_object.save()
...
def getchats(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        ...
        user_list = UserList.objects.filter(server=room,username=name)
        user_list_length = len(user_list)
        ...
        if user_list_length == 0:
            ...
            #update user
            user_list.update(time=datetime.datetime.now().astimezone())
...
def removeOldUsers():
    ...
    for i in UserList.objects.all().order_by('time').values():
        ...
        name = i.get('username')
        time = i.get('time')
        server_object = ServerList.objects.get(server=i.get('server_id'))
...
def removeUnusedServers():
    for i in ServerList.objects.all().order_by('lastUpdate').values():
        ...
        chat_list = Chats.objects.filter(server=server).values()
        user_list = UserList.objects.filter(server=server).values()

        chat_list_length = len(chat_list)
        user_list_length = len(user_list)

        #remove all servers where timeout has occurred
        if time <= t and chat_list_length == 0 and user_list_length == 0:
            server.delete()

        elif user_list_length == 0 and lock == True:
...
def validate_color(color):
    ...
    if color.lower() in color_dict:
        ...
        for i,char in enumerate(color):
            if char not in '0123456789ABCDEFabcdef' and i>0:
                isValid = False
